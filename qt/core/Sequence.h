...
class PROFILE_DLL_API Sequence final : public XMLTagHandler{
 public:
   ...
   void AppendAlias(const QString &fullPath,
                    sampleCount start,
   ...
   void HandleXMLEndTag(const QString &tag) override;
...
public:
   ...
   void ConsistencyCheck (const QString &whereStr, bool mayThrow = true) const;
   ...
   static void DebugPrintf
      (const BlockArray &block, sampleCount numSamples, QString *dest);
...
private:
   ...
   void CommitChangesIfConsistent
      (BlockArray &newBlock, sampleCount numSamples, const QString &whereStr);
