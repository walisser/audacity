...
protected:
   ...
   explicit MessageBoxException( const QString &caption = QString{} );
...
{
public:
   explicit SimpleMessageBoxException( const QString &message_,
      const QString &caption = _("Message") )
   ...
   try { return body(); }
      ...
      auto end = finally([&]{
         if (!std::uncaught_exception()) {
            ...
            //--} );
            qDelayedCall([=]() { // capture pException by value
               delayedHandler( pException.get() );
            });
         }
      });
