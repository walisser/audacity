...
ODDecodeBlockFile::ODDecodeBlockFile(const QString &baseFileName, const QString &audioFileName, sampleCount aliasStart,
                   size_t aliasLen, int aliasChannel,unsigned int decodeType)
try
   : SimpleBlockFile{ baseFileName,
...
ODDecodeBlockFile::ODDecodeBlockFile(const QString &existingFile, const QString &audioFileName, sampleCount aliasStart,
                     size_t aliasLen, int aliasChannel, unsigned int decodeType,
...
auto ODDecodeBlockFile::GetSpaceUsage() const -> DiskByteCount
{
   if(IsSummaryAvailable())
   {
      ...
      return QFileInfo(mFileName).size();
   }
   else
   {
      return 0;
...
BlockFilePtr ODDecodeBlockFile::Copy(const QString &newFileName)
{
...
// may throw
{
   ...
   else
   {
      xmlFile.StartTag(wxT("oddecodeblockfile"));
      {
         ...
         xmlFile.WriteAttr(wxT("summaryfile"), QFileInfo(mFileName).fileName());
      }
      xmlFile.WriteAttr(wxT("audiofile"), mAudioFileName);
...
BlockFilePtr ODDecodeBlockFile::BuildFromXML(DirManager &dm, const QStringMap &attrs)
{
        ...
        ++it)
   {
      ...
      else if (key == wxT("aliasstart"))
      {
         if (XMLValueChecker::IsGoodInt64(value) &&
             (nnValue = value.toLongLong(&ok)) >= 0 && ok)
            aliasStart = nnValue;
      }
      else if (XMLValueChecker::IsGoodInt(value) && (nValue=value.toLong(&ok), ok))
      {
         // integer parameters
         if (key == wxT("aliaslen") && (nValue >= 0))
            aliasLen = nValue;
         else if (key == wxT("aliaschannel") && XMLValueChecker::IsValidChannel(nValue))
            aliasChannel = nValue;
         else if (key == wxT("decodetype"))
            decodeType = nValue;
      }
   }
...
int ODDecodeBlockFile::WriteODDecodeBlockFile()
{
...

   {
      ...
      QMutexLocker locker(&mDecoderMutex);
      ...
      if(ret < 0) {
         qWarning("ODDecodeBlockFile Decode failure: %d", ret);
         return ret;
      }
   }

   {
      ...
      bool bSuccess = WriteSimpleBlockFile(sampleData.ptr(),mLen, mFormat, NULL);
      if ( !bSuccess )
         return -1;
   }
...
void ODDecodeBlockFile::SetFileName(const QString &name)
{
   mFileNameMutex.lock();
   mFileName=name;
...
///sets the file name the summary info will be saved in.  threadsafe.
QString ODDecodeBlockFile::GetFileName() const
{
   QMutexLocker locker(&mFileNameMutex);
   return mFileName;
}
...
{
   //I dont think we need to add a mutex here because only the main thread
   //changes filenames and calls ReadSummary
   if(IsSummaryAvailable())
...
{
   //since this is the only place that writes to mdecoder,
   //it is totally thread-safe to read check without the mutex
   if(decoder == mDecoder)
      return;
   mDecoderMutex.lock();
   mDecoder = decoder;
...
void ODDecodeBlockFile::ChangeAudioFile(const QString &newAudioFile)
{
