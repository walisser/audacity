...
class ODPCMAliasBlockFile final : public PCMAliasBlockFile
{
 public:
   ...
   ODPCMAliasBlockFile(const QString &baseFileName,
                        const QString &aliasedFileName, sampleCount aliasStart,
                        size_t aliasLen, int aliasChannel);
   ODPCMAliasBlockFile(const QString &existingSummaryFileName,
                        const QString &aliasedFileName, sampleCount aliasStart,
                        size_t aliasLen, int aliasChannel,
...
protected:
   ...
   //   sampleFormat format, ArrayOf<char> &summary) override;

  private:

   // TODO: can we get away with one mutex for everything?
    QMutex mWriteSummaryMutex;

   //need to protect this since it is changed from the main thread upon save.
   // mutable because we need to lock from overridden const methods
...
//    ODLock mAliasedFileNameMutex;
   ...
   // FIXME: libsndfile calls are all mutex locked anyways?
   mutable QMutex mReadDataMutex;

   mutable QMutex mSummaryAvailableMutex;
